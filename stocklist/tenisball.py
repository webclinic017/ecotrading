...
class PandasData(btfeed.PandasData):
    lines = ('pattern_rating', 'ma200','top','mavol' )  
    params = (
        ('datetime', 'date_time'),
        ('open', 'open'),
        ('high', 'high'),
...
class tenisball_otm(bt.SignalStrategy):
    params = (
        ('ma_backtest',dict_params['ma_backtest'] ),
        ('risk', dict_params['risk']),
        ('ratio_cutloss',dict_params['ratio_cutloss'] ),
        ('pattern_rating',dict_params['pattern_rating'] ),
        ('ratio_backtest',dict_params['ratio_backtest'] ),
    )
    def __init__(self, ticker, save_deal, strategy):
        ...
        self.sma = bt.indicators.SimpleMovingAverage(self.data.close, period=int(self.params.ma_backtest))
        self.trailing_sl = None  # Biến đồng hồ để lưu giá trị stop loss
        self.trailing_tp =None
        #điều kiện buy
        self.buy_trend = self.data.close > self.data.ma200
        self.buy_decrease = self.data.close < self.data.top
        self.buy_minvol =self.data.mavol > 100000
        self.buy_pattern= self.data.pattern_rating >= self.params.pattern_rating
        self.buy_backtest_ma = self.data.close >= self.sma *self.params.ratio_backtest
    def next(self):
        if len(self.data)>2:
            if not self.position:
                if self.buy_trend == True and self.buy_decrease==True and self.buy_minvol==True and self.buy_pattern == True and self.buy_backtest_ma ==True:
                    self.buy()
    ...
    #     if trade.justopened:
    #         print('----TRADE OPENED----')
    #         print('ma: {}'.format(sma[-1]))
    #         print('pattern: {}'.format(self.data.pattern_rating[-1]))
...
def evaluate_strategy(params,nav,commission,size_class,data,strategy_class, ticker, strategy, risk):
    cerebro = bt.Cerebro()
    cerebro.adddata(data)
    cerebro.broker.setcash(nav)  # Số dư ban đầu
    cerebro.broker.setcommission(commission=commission)  # Phí giao dịch (0.15%)
    #add the sizer
    cerebro.addsizer(size_class, risk=risk,ratio_cutloss =params[3])
    cerebro.addstrategy(strategy_class, 
                        ticker, False,strategy,
                        ma_backtest=params[0], 
                        ratio_backtest=params[1], 
                        ratio_cutloss =params[2],
                        pattern_rating = params[3],
                        )
    ...
    for item in stock_test[begin_list:end_list]:
        ...
        try:
            ...
            for params in param_combinations:
                ...
                try:
                    performance = evaluate_strategy(params,nav=strategy.nav,commission= strategy.commission,size_class = definesize,data= data,strategy_class = tenisball_otm,ticker =  ticker, strategy=strategy, risk =risk)
            ...
            if best_params:
                params_data = {
                        'ma_backtest': best_params[0],  
                        'ratio_backtest': best_params[1],  
...
def run_backtest_one_stock(ticker):
    strategy = StrategyTrading.objects.filter(name='tenisball_ver0.1',risk = 0.03).first()
    strategy_data = {
        'name': 'Breakout',
        'risk': 0.03,   
        'nav': 10000000,
